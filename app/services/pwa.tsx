...
export class BackgroundSyncManager {
  ...
  async registerBackgroundSync(tag: string): Promise<void> {
    ...
    try {
      ...
      if ("sync" in registration) {
        await (
          registration as { sync: { register: (tag: string) => Promise<void> } }
        ).sync.register(tag);
      }
    } catch (error) {
    ...
    for (const change of pendingChanges) {
      try {
        await this.syncChange(
          change as { type: string; data: unknown; id: string },
        );
  ...
  private async syncChange(_change: {
    type: string;
    data: unknown;
    id: string;
  }): Promise<void> {
...
export class PWAInstallManager {
  ...
  async showInstallPrompt(): Promise<boolean> {
    ...
    try {
      (
        this.deferredPrompt as {
          prompt: () => void;
          userChoice: Promise<{ outcome: string }>;
        }
      ).prompt();
      const { outcome } = await (
        this.deferredPrompt as { userChoice: Promise<{ outcome: string }> }
      ).userChoice;
      this.deferredPrompt = null;
...
export const PWA_CONFIG: PWAConfig = {
  ...
  shortName: "Navy PDM",
  description:
    "Comprehensive predictive maintenance system for Navy fleet management",
  themeColor: "#FF3621",
