...
async function installPythonDependencies() {
  ...
  const pipCommands = [
    ...
    ['python3', ['-m', 'pip', 'install', '-r', requirementsFile, '--quiet', '--break-system-packages']],
    ['python3', ['-m', 'pip', 'install', '-r', requirementsFile, '--quiet']],
  ];
  ...
  for (const [command, args] of pipCommands) {
    try {
      ...
      await runCommand(command, args, { cwd: backendDir, silent: true });
  ...
  console.log('⚠ Could not install via pip. Checking if packages are already available...');
  try {
